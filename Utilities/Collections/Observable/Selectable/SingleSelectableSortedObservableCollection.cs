...
public sealed class SingleSelectableSortedObservableCollection<T>
    : SelectableSortedObservableCollection<T>, ISingleSelectableObservableCollection<T>
    where T : class, INotifyPropertyChanged
{
    public SingleSelectableSortedObservableCollection()
    ...
    protected override void HandleLockReleased(object? sender, EventArgs args)
    {
        ...
        if (!Contains(selectedItem))
            ...
            // Item is still selected and present, but view may have lost sync. Force UI to rebind to valid value.
